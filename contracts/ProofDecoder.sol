...
library ProofDecoder {
    ...
    function decodeFullOutcomeProof(
        Borsh.Data memory data
    ) internal view returns (FullOutcomeProof memory proof) {
    ...
    function decodeBlockHeaderLight(
        Borsh.Data memory data
    ) internal view returns (BlockHeaderLight memory header) {
    ...
    function decodeExecutionOutcome(
        Borsh.Data memory data
    ) internal view returns (ExecutionOutcome memory outcome) {
        outcome.logs = new bytes[](data.decodeU32());
        for (uint256 i = 0; i < outcome.logs.length; i++) {
            outcome.logs[i] = data.decodeBytes();
        }

        uint256 start = data.ptr;
        outcome.receipt_ids = new bytes32[](data.decodeU32());
        for (uint256 i = 0; i < outcome.receipt_ids.length; i++) {
        ...
        outcome.merkelization_hashes[0] = Utils.sha256Raw(
            start,
            data.ptr - start
        );
        for (uint256 i = 0; i < outcome.logs.length; i++) {
    ...
    function decodeMerklePathItem(
        Borsh.Data memory data
    ) internal pure returns (MerklePathItem memory item) {
        item.hash = data.decodeBytes32();
        item.direction = data.decodeU8();
        require(
            item.direction < 2,
            "ProofDecoder: MerklePathItem direction should be 0 or 1"
        );
    }
    ...
    function decodeMerklePath(
        ...
        for (uint256 i = 0; i < path.items.length; i++) {
